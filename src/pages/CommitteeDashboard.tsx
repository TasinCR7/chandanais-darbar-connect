...
const CommitteeDashboard = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error) toast({ title: "লগইন ব্যর্থ", description: error.message, variant: "destructive" });
    } finally {
  ...
  const handleAddComment = async () => {
    ...
    setNewComment("");
    toast({ title: "মন্তব্য যোগ হয়েছে" });
    fetchData();
  };
  ...
  if (!user || !isCommitteeMember) {
    return (
      <>
        <SEO title="কমিটি সদস্য লগইন" description="কমিটি সদস্যদের ভোটিং ও মতামত প্যানেল" canonical="/committee-login" />
  ...
  return (
    <>
      <SEO title="কমিটি ড্যাশবোর্ড" description="ভোটিং ও মতামত প্রদান" canonical="/committee-login" />
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mb-10 gap-4 bg-card/40 backdrop-blur-md p-6 rounded-2xl border border-gold/20 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">কমিটি সদস্য</p>
              </div>
            </div>
          ...
          {loadingData ? <PremiumLoader /> : (
            <div className="space-y-10">
              ...
              <section>
                ...
                {topics.length === 0 ? (
                  <div className="text-center py-12 bg-card/30 rounded-2xl border border-gold/10">
                    ...
                    <p className="text-muted-foreground">এখনো কোনো ভোটের বিষয় নেই।</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {topics.map((topic) => {
                      ...
                      return (
                        <motion.div key={topic.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                          ...
                          {totalVotes > 0 && (
                            <div className="my-4">
                              <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
                                <span>সন্তুষ্ট {counts.satisfied}</span>
                                <span>অসন্তুষ্ট {counts.unsatisfied}</span>
                              </div>
                              <div className="h-2.5 bg-black/20 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full transition-all duration-500" style={{ width: `${satisfiedPct}%` }} />
                              </div>
                              <p className="text-[10px] text-gold/40 mt-1">মোট ভোট: {totalVotes}</p>
                            </div>
                          )}
                          ...
                          <div className="flex gap-3 mt-4">
                            ...
                            >
                              <ThumbsDown size={16} className="mr-2" /> অসন্তুষ্ট
                            </Button>
                          </div>
                        </motion.div>
                      );
              ...
              <section>
                <h2 className="text-xl font-heading font-bold text-gold mb-6 flex items-center gap-2">
                  <MessageSquare size={22} /> মতামত ও মন্তব্য
                </h2>
                ...
                <div className="bg-card/50 backdrop-blur border border-gold/15 rounded-2xl p-5 mb-6">
                  <Textarea
                    ...
                    placeholder="আপনার মতামত বা মন্তব্য লিখুন..."
                  ...
                  <Button onClick={handleAddComment} disabled={!newComment.trim()}
                    ...
                    <Send size={16} className="mr-2" /> মন্তব্য করুন
                  </Button>
                </div>
                ...
                <div className="space-y-3">
                  {comments.length === 0 ? (
                    <p className="text-center text-muted-foreground py-8">কোনো মন্তব্য নেই।</p>
                  ) : (
                    comments.map(c => (
