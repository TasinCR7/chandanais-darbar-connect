...
const Finance = () => {
  ...
  const loadAll = async () => {
    const [m, p, e, t, s] = await Promise.all([
      ...
      supabase.from('payments').select('*, members(full_name, member_code, area)').order('payment_date', { ascending: false }),
      supabase.from('expenses').select('*').order('expense_date', { ascending: false }),
      supabase.from('monthly_targets').select('*').order('for_year', { ascending: false }).order('for_month', { ascending: false }),
    ...
    (s.data ?? []).forEach((row: any) => { sObj[row.key] = row.value; });
    setSettings(sObj);
  };
  ...
  const downloadAreaPDF = () => {
    const monthOpt = areaScope === 'month' ? { month: reportMonth } : {};
    const slug = areaScope === 'month'
      ? `${ORG_NAME_SLUG}-area-${reportYear}-${String(reportMonth).padStart(2, '0')}`
      : `${ORG_NAME_SLUG}-area-${reportYear}`;
    downloadAreaReportPDF(members as any, payments as any, reportYear, {
      activeOnly, filename: reportFilename.trim() || slug, ...monthOpt,
    });
    toast({ title: `এলাকা ভিত্তিক PDF ডাউনলোড হয়েছে` });
  };
  ...
  return (
    <Layout>
      ...
      <div className="bg-gradient-to-r from-[hsl(43_70%_55%)] via-[hsl(38_75%_50%)] to-[hsl(43_70%_55%)] py-8">
        <div className="container max-w-7xl flex items-center justify-between gap-4 flex-wrap">
      ...
      <div className="container max-w-7xl py-8">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={duesTrend}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v: any) => [`৳ ${Number(v).toLocaleString('en-IN')}`, 'বকেয়া']}
                    />
                    <Legend />
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          outerRadius={90}
                          paddingAngle={2}
                          label={(e: any) => `${e.name}: ${toBanglaNumber(Math.round((e.percent ?? 0) * 100))}%`}
                        >
        ...
        {tab === 'ranking' && (
          <div className="card-gold rounded-2xl p-6 overflow-x-auto">
            <h3 className="font-display text-lg gold-text mb-4">শীর্ষ দাতা র‍্যাঙ্কিং 🏆</h3>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-primary/20 text-left font-bangla text-muted-foreground">
...
const StatCard = ({ icon, label, value, tone = 'gold' }: { icon: any; label: string; value: string; tone?: 'gold' | 'danger' }) => (
...
const PersonalView = ({ member, payments }: { member: any; payments: any[] }) => {
